...
/**
 * This class represents the customer. Each customer object is created with an id and table list.
 * totalConsumeTimeMS = eating time of the chosen food or beverage
 * totalPrepareTimeMS = chef's preparation time
 * totalTimeMS = total time spent at the table (totalConsumeTimeMS + totalPrepareTimeMS)
 * @author Merve Ozer
 */
...
public class Customer extends Thread implements FoodChooser, BeverageChooser, TableChooser{
	...
	public void run() {
		try {
			...
			System.out.println(this + " Total eating duration:" + this.totalConsumeTimeMS + ". Total waiting time:" + totalPrepareTimeMS);
			this.totalTimeMS = this.totalConsumeTimeMS + this.totalPrepareTimeMS;

			chosenTable.addCustomer(this);
			//chosenTable.addCustomer(this);	
			Thread.sleep(this.totalConsumeTimeMS);
